...
async function loadProduct(page, param, listcate) {
    ...
    if (listcate == null) {
        ...
        if (typeof listcate === "string") {
            listcate = listcate.split(",").map(Number);
        }
        console.log(listcate)
        var obj = {
    ...
    for (i = 1; i <= totalPage; i++) {
        mainpage += `<li onclick="loadProduct(${(Number(i) - 1)},'${param}',${listcate ? `'${listcate.join(",")}'` : 'null'})" class="page-item"><a class="page-link" href="#listsp">${i}</a></li>`
    }
